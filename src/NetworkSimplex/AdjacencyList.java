...
public class AdjacencyList implements Iterable<Arc> {
	 ...
	 * and not for a Network!! it looks for an arc with either startnode
	 * nodeIndex1 or startnode nodeIndex2 this is only. this is only well
	 * defined in a tree, bcz there exists only one such arc in a network there
	 * might be more!
	 * 
	 * @param nodeIndex1
	 * @param nodeIndex2
	...
	public Arc getEdgeInTree(int nodeIndex1, int nodeIndex2) {
		...
		if (arc != null)
			...
			return this.getEdge(nodeIndex2, nodeIndex1);
	}

	public Arc isInAdjList(int startNodeIndex, int endNodeIndex) {
		Iterator<Arc> iterator = this.iterator(startNodeIndex);
		Arc arc;
		while (iterator.hasNext()) {
			arc = iterator.next();
			if ((arc.getStartNodeIndex() == startNodeIndex)
					&& (arc.getEndNodeIndex() == endNodeIndex)) {
				return arc;
			}
		}
		return null;
